...
jest.mock('framer-motion', () => ({
  motion: {
    a: ({ children, ...props }: React.PropsWithChildren<any>) => (
      <a data-testid="motion-link" {...props}>{children}</a>
    ),
  },
}));
...
describe('Footer Component', () => {
  ...
  it('displays the correct copyright year', () => {
    ...
    expect(screen.getByText(new RegExp(`© ${currentYear} ${mockUserData.fullName}`))).toBeInTheDocument();
  });
  ...
  it('renders all social links', () => {
    ...
    expect(linkedinLink).toHaveAttribute('href', mockUserData.socialLinks[1].url);
